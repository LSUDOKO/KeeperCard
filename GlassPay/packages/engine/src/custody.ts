...
// Master key lives OUTSIDE the DB (env ATTESTPAY_MASTER_KEY, 32-byte hex; KMS post-hackathon).
...
async function masterKey(): Promise<CryptoKey> {
  const raw = process.env.ATTESTPAY_MASTER_KEY;
  if (!raw || !/^(0x)?[0-9a-fA-F]{64}$/.test(raw)) {
    throw new EngineError(
      "custody",
      "ATTESTPAY_MASTER_KEY must be set to 32-byte hex (generate: openssl rand -hex 32)",
    );
  }
