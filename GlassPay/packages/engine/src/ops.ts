...
export async function nukeAll(deps: OpsDeps, userId: string): Promise<AdminOpResult & { newNonce: bigint }> {
  ...
  const nukedCards = deps.store.listCards(userId).filter((c) => c.status === "active" || c.status === "frozen").length;
  activeCards.add(-nukedCards);
...
export async function finalizeAdminOp(
  ...
  if (prepared.kind === "revoke") {
    ...
    const subtreeCount = 1 + deps.store.subtreeIds(prepared.cardId!).length;
    activeCards.add(-subtreeCount);
    return result;
  }
  ...
  const nukedCards = deps.store.listCards(prepared.userId).filter((c) => c.status === "active" || c.status === "frozen").length;
  activeCards.add(-nukedCards);
