...
//   Lane C (OAuth 2.1, additive Jun 7 2026): /mcp + Bearer glsp_at_<...> access token
//          minted by the self-hosted AS (oauth/routes.ts) after card-picker consent.
//          The glsp_at_ prefix disambiguates from card secrets without a second lookup.
...
import { hashCardSecret, type CardRow } from "@attestpay/engine";
...
export function mcpRoutes(deps: AppDeps, oauth: OAuthStore): Hono {
  ...
  const badSecret = new RateLimiter(envInt("ATTESTPAY_MCP_BAD_SECRET_LIMIT", 30), 60_000);
  // per-card request ceiling: 240 calls / minute (well above any real agent loop)
  const perCard = new RateLimiter(envInt("ATTESTPAY_MCP_RATE_LIMIT", 240), 60_000);

  // host allowlist (DNS-rebinding): enforced only when an explicit public base is set.
  // ATTESTPAY_ALLOWED_HOSTS (comma-separated) widens it, e.g. the Railway fallback domain.
  // Evaluated per request (cheap) so env changes don't require a route rebuild.
  const allowedHosts = (): Set<string> | null => {
    try {
      const fromBase = process.env.ATTESTPAY_PUBLIC_MCP_BASE ? [new URL(process.env.ATTESTPAY_PUBLIC_MCP_BASE).host] : [];
      const extra = (process.env.ATTESTPAY_ALLOWED_HOSTS ?? "").split(",").map((h) => h.trim()).filter(Boolean);
  ...
  const serve = async (c: Context, credential: string | undefined, oauthLane: boolean) => {
    ...
    // resolve the credential to a card. OAuth access tokens (glsp_at_ prefix) are only
    ...
    // fallback: a card secret that happens to start with the glsp_at_ prefix (astronomically
