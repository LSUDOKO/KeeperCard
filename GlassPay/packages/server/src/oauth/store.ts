...
import { hashCardSecret } from "@attestpay/engine";
...
export const ACCESS_TOKEN_PREFIX = "glsp_at_";
export const REFRESH_TOKEN_PREFIX = "glsp_rt_";
...
export class OAuthStore {
  ...
  createClient(c: { redirectUris: string[]; clientName?: string | null }): OAuthClientRow {
    const clientId = `glsp_client_${crypto.randomUUID()}`;
  ...
  createCode(c: Omit<OAuthCodeRow, "code_hash" | "used" | "expires_at">, ttlSeconds: number): string {
    const code = randomToken("glsp_code_");
    const now = Math.floor(Date.now() / 1000);
