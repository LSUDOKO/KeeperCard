...
// PKCE and mints opaque glsp_at_/glsp_rt_ tokens. RFC 8707 `resource` is round-tripped
...
export function publicBase(): string {
  const raw = process.env.ATTESTPAY_PUBLIC_MCP_BASE ?? `http://localhost:${process.env.PORT ?? 4070}`;
  return raw.replace(/\/+$/, "");
}
...
const dashboardBase = (): string =>
  (process.env.ATTESTPAY_DASHBOARD_BASE ?? "http://localhost:4071").replace(/\/+$/, "");
 ...
 *  - https: any host (optionally restricted via ATTESTPAY_OAUTH_REDIRECT_HOSTS),
...
function redirectUriAllowed(uri: string): boolean {
  ...
  if (url.protocol === "https:") {
    const restrict = (process.env.ATTESTPAY_OAUTH_REDIRECT_HOSTS ?? "").split(",").map((s) => s.trim()).filter(Boolean);
...
export const acceptedResources = (): string[] => {
  const extra = (process.env.ATTESTPAY_OAUTH_ACCEPTED_RESOURCES ?? "")
    .split(",")
    .map((s) => s.trim())
    .filter(Boolean);
...
export function oauthRoutes(deps: AppDeps, oauth: OAuthStore): Hono {
  ...
  const accessTtl = () => envInt("ATTESTPAY_OAUTH_ACCESS_TTL", 3600);
  const refreshTtl = () => envInt("ATTESTPAY_OAUTH_REFRESH_TTL", 30 * 86_400);

  const registerLimit = new RateLimiter(envInt("ATTESTPAY_OAUTH_REGISTER_LIMIT", 10), 60_000);
  const authorizeLimit = new RateLimiter(envInt("ATTESTPAY_OAUTH_AUTHORIZE_LIMIT", 30), 60_000);
  const tokenLimit = new RateLimiter(envInt("ATTESTPAY_OAUTH_TOKEN_LIMIT", 60), 60_000);
