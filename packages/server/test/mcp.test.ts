...
describe("fiat tools", () => {
  ...
  beforeAll(() => {
    ...
    // widen the host allowlist to this second server (ATTESTPAY_PUBLIC_MCP_BASE pins the first)
    process.env.ATTESTPAY_ALLOWED_HOSTS = new URL(fiatBase).host;
  });

  afterAll(() => {
    fiatServer.stop(true);
    delete process.env.ATTESTPAY_ALLOWED_HOSTS;
  });
...
describe("mcp hardening", () => {
  test("Host header outside the allowlist is rejected (421); allowed host passes", async () => {
    ...
    // ATTESTPAY_PUBLIC_MCP_BASE = base, so the test server's own host is allowed
