...
import { FEE_COLLECTOR, KeyedMutex, Store, issueRootCard, issueSubCard, freezeCard, unfreezeCard, type Relayer } from "@attestpay/engine";
...
beforeAll(async () => {
  ...
  process.env.ATTESTPAY_STRIPE_WEBHOOK_SECRET = WHSEC;
...
afterAll(() => {
  server.stop(true);
  delete process.env.ATTESTPAY_STRIPE_WEBHOOK_SECRET;
});
...
describe("the 2s decision", () => {
  ...
  test("frozen card declines; unknown mapping declines; non-usd declines", async () => {
    ...
    const { unfreezeCard } = await import("@attestpay/engine");
...
describe("fiat charges + settlement mode", () => {
  ...
  test("settlement mode: approved row lands pending with a recipient and fires the settler", async () => {
    ...
    process.env.ATTESTPAY_FIAT_SETTLEMENT = "1";
    delete process.env.ATTESTPAY_SETTLEMENT_ADDRESS; // recipient falls back to the fee collector
    deps.fiatSettler = {
      settle: async (cid) => {
        settles.push(cid);
      },
    ...
    try {
      ...
      delete process.env.ATTESTPAY_FIAT_SETTLEMENT;
