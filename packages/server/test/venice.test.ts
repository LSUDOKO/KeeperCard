...
import { KeyedMutex, Store, type Relayer } from "@attestpay/engine";
...
describe("/cards/compile route", () => {
  ...
  const savedMasterKey = process.env.ATTESTPAY_MASTER_KEY;
  beforeAll(() => {
    process.env.ATTESTPAY_MASTER_KEY = "e".repeat(64);
  });
  afterAll(() => {
    if (savedMasterKey === undefined) delete process.env.ATTESTPAY_MASTER_KEY;
    else process.env.ATTESTPAY_MASTER_KEY = savedMasterKey;
  });
